...
- **منصة الموارد البشرية** عبر البوابة `/` ثم `/hr` — رفع ملفات الإكسل والتحليلات. عند ضبط
  MySQL، **كل ملف ترفعه يُحفظ تلقائياً في قاعدة البيانات** ويبقى بعد إعادة
  النشر (لا يعود للبيانات القديمة).
- **نظام التقييم** على `/evaluation` — تقييم الأداء + **روابط التقييم** التي
  ترسلها للمدير المباشر (`/evaluation/e/<token>`).
...
5. **Settings → Networking → Generate Domain** ثم افتح الرابط.
   ...
   - نظام التقييم: `https://<domain>/evaluation/`
