...
(function () {
  ...
  Pages.upload = function (content) {
    ...
    content.innerHTML = h('ارفع ملف الموظفين الشهري — القراءة تتم بالكامل داخل متصفحك، بدون خادم') + steps(1) +
      ...
      <div class="data-note">كل البيانات تأتي من ملف Excel الذي ترفعه — لا شيء مُسجّل مسبقاً. أي ملف ترفعه <b>يستبدل</b> البيانات الحالية بالكامل ويحدّث كل الشاشات فوراً (لوحة المعلومات، السعودة، الإقامات، الشهادات الصحية، الشركات المتعاقدة، والتقارير، وحتى البحث عن الموظفين) — حتى لو كان الملف يحتوي موظفاً واحداً فقط. ويُحفظ في هذا المتصفح فيبقى بعد تحديث الصفحة أو إغلاقها. (الحفظ خاص بكل متصفح/جهاز — ارفع ملفك مرة على الجهاز الذي تستخدمه.)</div>`;
    const bindMgmt = () => {
